...
#include "esp_sleep.h"
#define SLEEP_TIME 540000000   // sleep for 9 minutes

static const char* LOG_TAG = "open_haystack";

void vTimerCallback() {
    ESP_LOGI(LOG_TAG, "Start sleeping for 9 minutes.");
...
void app_main(void)
{
    ...
    double interval = pdMS_TO_TICKS(60000); // set for 1 minute
    timerHndl1Sec = xTimerCreate("MyTimer", interval, pdTRUE, ( void * )0, vTimerCallback);
    if (xTimerStart(timerHndl1Sec, 0)!=pdPASS) {
        ESP_LOGI(LOG_TAG, "Timer not set.");
    } else {
        ESP_LOGI(LOG_TAG, "Set timer for 1 minute.");
    }
}
